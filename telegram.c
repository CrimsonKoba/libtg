...
char *contact_names[] = {"phone_number",
			 "first_name",
			 "last_name",
			 "user_id",
			 NULL};
...
char *chat_type_names[] = {"private",
			   "group",
			   "supergroup",
			   "channel",
			   NULL};

char *sticker_names[] = {"file_id",
			 "width",
			 "height",
			 "thumb",
			 "file_size",
...
char *photosize_names[] = {"file_id",
			   "width",
			   "height",
			   "file_size",
			   NULL};

char *voice_names[] = {"file_id",
		       "duration",
		       "mime_type",
		       "file_size",
		       NULL};

char *location_names[] = {"longitude",
			  "latitude",
			  NULL};

char *document_names[] = {"file_id",
			  "thumb",
			  "file_name",
			  "mime_type",
			  "file_size",
...
char *update_names[] = {"update_id",
			"message",
			"inline_query",
			"chosen_inline_result",
			NULL};

char *file_names[] = {"file_id",
		      "file_size",
		      "file_path",
		      NULL};

int message_states[3][5] = {{4, 1, 4, 4, 4},
			    {4, 4, 4, 2, 4},
			    {4, 1, 1, 1, 1}};

int parse_array_states[2][5] = {{4, 4, 1, 4, 4},
				{4, 1, 4, 4, 4}};
	...
	while (entrar && i < r) {
		...
		case 2:
			...
			case 1:
				...
				telegram_error_buffer =
				    calloc(strlen(value) + 2, sizeof(char));
				memcpy(telegram_error_buffer, value,
			...
			default:
				telegram_set_error(
				    ...
				    TELEGRAM_ERROR_UNEXPECTED_TOKEN_VALUE);
				entrar = 0;
				break;
			}
			free(variable);
		...
		case 3:
			telegram_set_error(
			    ...
			    TELEGRAM_ERROR_UNEXPECTED_TOKEN_TYPE);
			entrar = 0;
			break;
		default:
			break;
...
File *telegram_getFile(char *file_id)
{
	...
	if (file_id) {
		...
		if (res == CURLE_OK) {
			...
			if (!telegram_error) {
				file =
				    telegram_parse_file(response->result, &i);
			}
			telegram_free_response(response);
			telegram_reset_buffer();
		} else {
			telegram_set_error("curl_easy_perform() failed!",
					   -1000);
		}
		free(postdata);
		curl_easy_cleanup(curl);
...
char *telegram_makeurl(char *telegram_method)
{
	...
	if (url != NULL) {
		snprintf(url, l + 1, "%s%s%s", telegram_baseurl, telegram_token,
			 telegram_method);
	} else {
...
size_t write_callback(void *ptr, size_t size, size_t nmemb, void *userdata)
{
	...
	if (new_len >= telegram_buffer_size) {
		...
		telegram_buffer = realloc(
		    telegram_buffer, (telegram_buffer_size + 2) * sizeof(char));
	...
	if (res == CURLE_OK) {
		...
		if (!telegram_error) {
			if (response && response->result) {
				updates = telegram_parse_updates(
				    response->result, &i);
			}
		}
		telegram_free_response(response);
	...
	while (entrar && i < r) {
		...
		case 1:
			...
			updates->list[updates->length] =
			    telegram_parse_update(value, &i);
			updates->length++;
			updates->list =
			    realloc(updates->list,
				    (updates->length + 2) * sizeof(Update *));
			break;
		case 4:
...
Update *telegram_parse_update(char *str, int *count)
{
	...
	while (entrar && i < r) {
		...
		case 2:
			...
			case 1:
				update->item.message =
				    telegram_parse_message(value, &i);
				free(value);
				break;
			case 2:
			case 3:
...
Message *telegram_parse_message(char *str, int *count)
{
	...
	while (entrar && i < r) {
		...
		case 2:
			...
			case 9: // document
				message->document =
				    telegram_parse_document(value, &i);
				free(value);
				break;
			case 10: // photo
				message->photo =
				    telegram_parse_photos(value, &i);
				free(value);
				break;
			case 11:
				message->sticker =
				    telegram_parse_sticker(value, &i);
				free(value);
				break;
			case 12: // video
				message->video =
				    telegram_parse_video(value, &i);
				break;
			case 13: // voice
				message->voice =
				    telegram_parse_voice(value, &i);
				free(value);
				break;
			case 14: // Cption
			...
			case 15: // contact
				message->contact =
				    telegram_parse_contact(value, &i);
				free(value);
				break;
			case 16: // location
				message->location =
				    telegram_parse_location(value, &i);
				free(value);
				break;
...
Sticker *telegram_parse_sticker(char *str, int *count)
{
	...
	while (entrar && i < r) {
		...
		case 2:
			...
			case 3:
				sticker->thumb =
				    telegram_parse_photosize(value, &i);
				free(value);
				break;
			case 4:
...
Photos *telegram_parse_photos(char *str, int *count)
{
	...
	while (entrar && i < r) {
		...
		case 1:
			...
			photos->item =
			    realloc(photos->item,
				    (photos->length + 2) * sizeof(PhotoSize *));
			break;
		case 4:
			telegram_dump_token(toks[i], str);
			telegram_set_error(
...
char *telegra_makeDownloadURL(File *file)
{
	...
	if (temp) {
		snprintf(temp, l + 3, "%s%s/%s", telegram_fileurl,
			 telegram_token, file->file_path);
...
char *telegram_downloadFile(File *file, char *name)
{
	...
	if (res == CURLE_OK) {
		if (file->file_size == telegram_buffer_offset) {
			...
			if (toLocalFile) {
				if (fwrite(telegram_buffer, sizeof(char),
	...
	if (filename) {
		...
		while (variables[i] != NULL && valores[i] != NULL) {
			curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME,
				     variables[i], CURLFORM_COPYCONTENTS,
				     valores[i], CURLFORM_END);
			i++;
		}
		curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME, "document",
		...
		if (res == CURLE_OK) {
			response = telegram_parse_response(telegram_buffer, &i);
			if (!telegram_error) {
				message = telegram_parse_message(
				    response->result, &i);
			} else {
